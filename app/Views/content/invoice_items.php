    ...
    <div class="col-md-12 container">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Invoice Items (Invoice No: <?=$invoice_no?>)</h6>
            </div>
            <div class="card-body">
                <button type="button" class="btn btn-primary btn-md" data-toggle="modal" data-target="#modelId"><i class="fa fa-plus" aria-hidden="true"></i> Add Invoice Item</button>                <hr>
                <div class="table-responsive">
                        ...
                        <?php if(!empty($items)): ?>
							<?php foreach($items as $key=>$item): ?>
								<tr>
									...
									<td><a href="<?=base_url('invoices/delete_invoice_item/'.$item->id.'/'.$invoice_no)?>" class="btn btn-danger btn-sm">Remove</a>
                    ...
                    <a href="<?=base_url('invoices/tax_and_discounts/'.$invoice_no)?>" class='btn btn-md btn-success'>Done, Proceed to Tax and discounts</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $('.td').on('click',function(){
        var id=$(this).attr('id');
        $.post("<?=base_url('invoices/fetch_invoice_item')?>", {id:id},
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?=base_url('invoices/invoice_items_save_edit/'.$invoice_no)?>" method="post">
...
<div class="modal fade" id="modelId" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">New Invoice Item</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
            </div>
            <div class="modal-body">
				<form action="<?=base_url('invoices/invoice_items_save/'.$invoice_no)?>" method="post">
                    <div class="form-group">
                        <label for="">Part Name:</label>
...
<script>
    function fetchstock() {
        ...
        $.post("<?=base_url('invoices/fetch_inventory')?>", data,
            function (data, textStatus, jqXHR) {
                if(data.success){
